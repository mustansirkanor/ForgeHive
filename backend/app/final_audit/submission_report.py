...
def build_demo_script(package: dict) -> str:
    ...
    return "\n".join(
        [
            ...
            "Show reward score, bandit/RL prior, Knowledge Graph relevance, Experience Graph prior, and selected bundle.",
            "",
            "## 6. Experience Graph Memory",
            "Show similar past situations, preferred historical strategy, actions to prefer, actions to avoid, and the newly stored episode.",
            "",
            "## 7. Safety Governor Approval",
            "Show final Safety Governor approval and rejected-action handling before execution.",
            "",
            "## 8. IDF Adapter Changes",
            ...
            "",
            "## 9. Digital Twin Execution Result",
            ...
            f"Experience Graph updated: {demo.get('experience_graph_updated')}",
            "",
            "## 11. Final Dashboard",
...
def build_judge_summary(package: dict) -> str:
    ...
    return "\n".join(
        [
            ...
            "",
            "ForgeHive includes an Experience Graph that stores previous EnergyPlus-tested operational episodes. Similar future building situations retrieve these memories, allowing the LLM and RL ranker to prefer historically successful strategies and avoid known failures while the Safety Governor remains the final authority.",
            "",
